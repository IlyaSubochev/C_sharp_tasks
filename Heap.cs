...
namespace AlgorithmsDataStructures2
{
    public class Heap
    {
        ...
        private int LastValueIndex() //Находит ячейку >0, в ином случае -1
        {
            int index = HeapArray.Length - 1;
            for (; index > -1 && HeapArray[index] == 0; index--) { }
            return index;
        }
        public bool Add(int key) // добавляем новый элемент key в кучу и перестраиваем её,  если куча вся заполнена
        {
            if (HeapArray == null)
                return false;
