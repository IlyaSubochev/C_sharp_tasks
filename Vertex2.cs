...
namespace AlgorithmsDataStructures2
{
    ...
    public class SimpleGraph<T>
    {
        ...
        public List<Vertex<T>> DepthFirstSearch(int VFrom, int VTo) // Узлы задаются позициями в списке vertex. Возвращается список узлов -- путь из VFrom в VTo. Список пустой, если пути нету.
        {
            ClearStackData();
            List<Vertex<T>> list = new List<Vertex<T>>();
            for (int i = 0; i < max_vertex; i++) 
                vertex[i].Hit = false;
            if (NextVert(VFrom, VTo)) 
        ...
        private bool NextVert(int VPres, int VTo) //  Добавляет указанную вершину в стек.Проводит проверку внутри цикла Cyc.
        {
            if (AVS(VPres) && Cyc(0, VPres, VTo)) 
                return true;
            return RVS();
        }

        private List<Vertex<T>> StackToList(List<Vertex<T>> list) // Копирует вершины из стека в список
        {
            for (int i = stack.dyn.count - 1; i > -1; i--) 
                list.Add(stack.dyn.array[i]);
            return list;
        }

        // -- Проверяем на наличие ребра у указанных вершин - ожидается true
        ...
        private bool Cyc(int i, int VPres, int VTo)
        {
            if (IsOver(i)) 
                return false;
            else if (!vertex[i].Hit && IsEdge(VPres, i) && (i == VTo || NextVert(i, VTo))) 
                return true;
            return Cyc(i + 1, VPres, VTo);
        }

       
        private bool AVS(int i)  // Добавляем Vertex в стек
        {
            vertex[i].Hit = true;
            stack.Push(vertex[i]);
            return true;
        ...
        private bool RVS() //Удаляем Vertex из стека
        {
            stack.Pop();
            return false;
        }

        private bool IsOver(int i) // проверка на окончание цикла
        {
            return i >= max_vertex;
        }
    }
