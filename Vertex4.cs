...
namespace AlgorithmsDataStructures2
{
    ...
    public class SimpleGraph<T>
    {
        ...
        private bool Cyc(int i, int VPres, int VTo)
        {
            if (IsOver(i)) 
                return false;
            else if (!vertex[i].Hit && IsEdge(VPres, i) && (i == VTo || NextStackVert(i, VTo))) 
                return true;
        ...
        {
            if (IsNotEmpty(index)) 
                return new Trace<Vertex<T>>(index);
            return null;
        }
        ...
        private Trace<Vertex<T>> NextQueueVert(Trace<Vertex<T>> step, int VTo)
        {
            ...
            for (int i = 0; i < max_vertex; i++)
                ...
                {
                    if (i == VTo) 
                        return step.SetNextTrace(step, i);
                    vertex[i].Hit = true;
